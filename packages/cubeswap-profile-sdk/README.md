# Cubeswap Profile SDK

This package provides some handy functions to retrieve data for Cubeswap Profile system.
...
Install `@cubeswap/profile-sdk` into your project with npm:

```bash
npm install @cubeswap/profile-sdk --save
```

or yarn:

```bash
yarn add @cubeswap/profile-sdk
```
...
```js
import CubeswapProfileSdk from "@cubeswap/profile-sdk";

const cubeswapSdk = new CubeswapProfileSdk();
```
...
- `chainId` - what chain ID to use, if not provided defaults to `1818`
...
const cubeswapSdk = new CubeswapProfileSdk(customProvider, 97);
```
...
const cubeswapSdk = new CubeswapProfileSdk();
const username = cubeswapSdk.getUsername("0x123456789");
...
const cubeswapSdk = new CubeswapProfileSdk();
const team = cubeswapSdk.getTeam(1);
console.log(team);
// {
//   id: 1,
//   name: "Syrup Storm",
...
It also sets `profile_${address}` cookie containing username and avatar (now only for cubeswap.finance domain, maybe configurable in future versions)

```js
import CubeswapProfileSdk from "@cubeswap/profile-sdk";

const cubeswapSdk = new CubeswapProfileSdk();
const profile = cubeswapSdk.getProfile("0x123456789");
console.log(profile);
// {
//   hasRegistered: true
//   profile: {
...
const achievements = cubeswapSdk.getAchievements("0x123456789");
...
```JSX
import { Team, teamImages } from "@cubeswap/profile-sdk";
...
Current version of this SDK is 90% copy of existing from [cubeswap-frontend](https://github.com/Cubeswap-exchange/cubeswap-frontend) repo. There are several improvements to be made in the future versions of this SDK:
