...
// Default token list for exchange + manual exclusion of broken CRC-20 token(s)
const blacklist: string[] = [
  // List of default tokens to exclude
  "0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c", // WBNB
  "0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82", // CSE
...
/**
 * Fetch Top100 Tokens traded on CubeswapSwap v2, ordered by trading volume,
    ...
    const { ethereum } = await request(
      ...
      gql`
        query ($from: ISO8601DateTime, $till: ISO8601DateTime, $blacklist: [String!]) {
          ethereum(network: cube) {
            dexTrades(
              options: { desc: "Total_USD", limit: 100 }
              exchangeName: { is: "Cubeswap v2" }
...
const getTokenLogo = (address: string): string => {
  ...
  if (logoFiles.includes(`${address}.png`)) {
    return `https://tokens.cubeswap.finance/images/${address}.png`;
  }
...
const main = async (): Promise<void> => {
  try {
    ...
    const tokenListPath = `${path.resolve()}/src/tokens/cubeswap-top-100.json`;
